...
/**
 * 
 * @author Leonid Andreev original author:
 * @author a.sone
 */
@Slf4j
public class TabularSubsetGenerator {
    ...
    private void bufferMoreColumnBytes(final int column) throws IOException {
        ...
        if (columnTotalLengths[column] < columnTotalOffsets[column] + MAX_COLUMN_BUFFER) {
            log.trace("Limiting the buffer to " + (columnTotalLengths[column] - columnTotalOffsets[column]) + " bytes");
        ...
        log.trace("Read " + columnBufferSizes[column] + " bytes for subset column " + column);
        columnBufferOffsets[column] = 0;
        columnTotalOffsets[column] += columnBufferSizes[column];
    }

    public byte[] readColumnEntryBytes(final int column) {
        return readColumnEntryBytes(column, true);
    }

    public byte[] readColumnEntryBytes(final int column, final boolean addTabs) {
        byte[] leftover = null;
        byte[] ret = null;
        ...
        try {
            while (columnByteBuffers[column].array()[byteindex] != '\n') {
                ...
                if (byteindex == columnBufferSizes[column]) {
                    ...
                    if (leftover == null) {
                        ...
                        System.arraycopy(columnByteBuffers[column].array(), columnBufferOffsets[column], leftover, 0,
                                columnBufferSizes[column] - columnBufferOffsets[column]);
                    } else {
                        byte[] merged = new byte[leftover.length + columnBufferSizes[column]];

                        System.arraycopy(leftover, 0, merged, 0, leftover.length);
                        System.arraycopy(columnByteBuffers[column].array(), 0, merged, leftover.length,
                                columnBufferSizes[column]);
                        leftover = merged;
                        merged = null;
                    }
                    // read more bytes:
            ...
            if (leftover == null) {
                ...
                System.arraycopy(columnByteBuffers[column].array(), columnBufferOffsets[column], ret, 0,
                        byteindex - columnBufferOffsets[column] + 1);
            } else {
    ...
    public void subsetFile(final String infile, final String outfile, final Set<Integer> columns, final Long numCases) {
    ...
    public void subsetFile(final InputStream in, final String outfile, final Set<Integer> columns, final Long numCases,
            final String delimiter) {
    ...
    public static Double[] subsetDoubleVector(final InputStream in, final int column, final int numCases) {
    ...
    /*
     * Straightforward method for subsetting a tab-delimited data file,
     * extracting all the columns representing continuous variables and
     * returning them as a 2-dimensional array of Doubles; Inefficient on large
     * files, OK to use on small ones.
     */
    public static Double[][] subsetDoubleVectors(final InputStream in, final Set<Integer> columns, final int numCases)
            throws IOException {
    ...
    public String[] subsetStringVector(final TabularDataIngest datafile, final int column) throws IOException {
        return (String[]) subsetObjectVector(datafile, column, COLUMN_TYPE_STRING);
    }

    public Double[] subsetDoubleVector(final TabularDataIngest datafile, final int column) throws IOException {
        return (Double[]) subsetObjectVector(datafile, column, COLUMN_TYPE_DOUBLE);
    }

    public Long[] subsetLongVector(final TabularDataIngest datafile, final int column) throws IOException {
    ...
    public Float[] subsetFloatVector(final TabularDataIngest datafile, final int column) throws IOException {
        return (Float[]) subsetObjectVector(datafile, column, COLUMN_TYPE_FLOAT);
    }

    public String[] subsetStringVector(final File tabfile, final int column, final int varcount, final int casecount)
            throws IOException {
        return (String[]) subsetObjectVector(tabfile, column, varcount, casecount, COLUMN_TYPE_STRING);
    }

    public Double[] subsetDoubleVector(final File tabfile, final int column, final int varcount, final int casecount)
            throws IOException {
        return (Double[]) subsetObjectVector(tabfile, column, varcount, casecount, COLUMN_TYPE_DOUBLE);
    }

    public Long[] subsetLongVector(final File tabfile, final int column, final int varcount, final int casecount)
            throws IOException {
        return (Long[]) subsetObjectVector(tabfile, column, varcount, casecount, COLUMN_TYPE_LONG);
    }

    public Float[] subsetFloatVector(final File tabfile, final int column, final int varcount, final int casecount)
            throws IOException {
        return (Float[]) subsetObjectVector(tabfile, column, varcount, casecount, COLUMN_TYPE_FLOAT);
    }

    public Object[] subsetObjectVector(final TabularDataIngest dataFile, final int column, final int columntype)
            throws IOException {
        ...
        File tabfile = dataFile.getTabDelimitedFile();
        if (!tabfile.exists()) {
    ...
    public Object[] subsetObjectVector(final File tabfile, final int column, final int varcount, final int casecount,
            final int columntype) throws IOException {
        return subsetObjectVector(tabfile, column, varcount, casecount, columntype, false);
    }

    public Object[] subsetObjectVector(final File tabfile, final int column, final int varcount, final int casecount,
            final int columntype, final boolean compatmode) throws IOException {
            ...
            while (bytesReadTotal < columnLength) {
                ...
                while (bytecount < bytesRead) {
                    if (columnBytes[bytecount] == '\n') {
                        /*
                         * String token = new String(columnBytes, byteoffset,
                         * bytecount-byteoffset, "UTF8");
                         * 
                         * if (leftover != null) { String leftoverString = new
                         * String (leftover, "UTF8"); token = leftoverString +
                         * token; leftover = null; }
                         */
                        /*
                         * Note that the way I was doing it at first - above -
                         * was not quite the correct way - because I was
                         * creating UTF8 strings from the leftover bytes, and
                         * the bytes in the current buffer *separately*; which
                         * means, if a multi-byte UTF8 character got split in
                         * the middle between one buffer and the next, both
                         * chunks of it would become junk characters, on each
                         * side! The correct way of doing it, of course, is to
                         * create a merged byte buffer, and then turn it into a
                         * UTF8 string. -- L.A. 4.0
                         */
                        String token = null;
                        ...
                        if (isString) {
                            ...
                            try {
                                ...
                                log.warn("NumberFormatException thrown for " + token + " as Double");
                            ...
                            try {
                                ...
                                log.warn("NumberFormatException thrown for " + token + " as Float");
    ...
    private long[] extractColumnOffsets(final File rotatedImageFile, final int varcount, final int casecount)
            throws IOException {
    ...
    private File getRotatedImage(final File tabfile, final int varcount, final int casecount) throws IOException {
    ...
    private File generateRotatedImage(final File tabfile, final int varcount, final int casecount) throws IOException {
        ...
        try (Scanner scanner = new Scanner(tabfileStream)) {
            scanner.useDelimiter("\\n");
     ...
     * edu.harvard.iq.dataverse.dataaccess.TabularSubsetGenerator testfile.tab
     * varcount casecount column type make sure the CLASSPATH contains ...
     * 
     */
    ...
    public static void main(final String[] args) {
        ...
        /*
         * try { rotatedImageFile = subsetGenerator.getRotatedImage(tabFile,
         * varcount, casecount); } catch (IOException ex) {
